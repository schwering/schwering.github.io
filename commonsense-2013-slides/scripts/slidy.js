/* slidy.js
   ...
   I've annotated changes made by myself (Christoph Schwering) by `chs'.
*/
...
var w3c_slidy = {
  ...
  enable_toolbar_links: false, // chs: en/disable `contents' and `help' links
  enable_initial_prompt: false, // chs: en/disable help prompt at that fades out

  show_slide_handler: null, // chs: handlers can be used for things like video.play()/pause() on the slide
  hide_slide_handler: null, // chs: use set_[hide|show]_slide_handler(function (slide) { ... })
  show_node_handler: null, // chs: these handlers are called when the next/previous node on the slide is
  hide_node_handler: null, // chs: added/removed, use set_[show|hide]_node_handler(function (slide, node) { ... })
  resize_handler: null, // chs: handler can be used to re-typeset MathJax stuff

  display_time: true, // chs: display time in the toolbar
  ...
  init: function () {
    ...
    if (this.slides.length > 0)
    {
      ...
      var hidden_nodes = new Array();

      if (this.slide_number > 0)
      {
        shown_nodes = this.set_visibility_all_incremental("visible");
      ...
      else
      {
        ...
        hidden_nodes = this.set_visibility_all_incremental("hidden");
      ...
      if (w3c_slidy.hide_node_handler) {
        for (var i = 0; i < hidden_nodes.length; ++i) {
          w3c_slidy.hide_node_handler(hidden_nodes[i]);
        }
      }
      if (w3c_slidy.show_node_handler) {
        for (var i = 0; i < shown_nodes.length; ++i) {
          w3c_slidy.show_node_handler(shown_nodes[i]);
        }
      }
    }
    ...
    if (w3c_slidy.enable_initial_prompt)
  ...
  toc_click: function (e) {
    ...
    if (target && target.nodeType == 1)
    {
      ...
      if (uri)
      {
        ...
        var hidden_nodes = w3c_slidy.set_visibility_all_incremental("hidden");
  ...
  show_all_slides: function () {
    ...
    var shown_nodes = this.set_visibility_all_incremental("visible");
    if (w3c_slidy.show_node_handler) {
      for (var i = 0; i < shown_nodes.length; ++i) {
        w3c_slidy.show_node_handler(shown_nodes[i]);
      }
  ...
  add_toolbar: function () {
    var counter, page;
     ...
     if (this.ns_pos || !this.ie6)
     {
       ...
       if (w3c_slidy.enable_toolbar_links) {
  ...
  collect_slides: function () {
    ...
    for (var i = 0; i < divs.length; ++i)
    {
      ...
      if (this.has_class(div, "slide"))
      {
        ...
        w3c_slidy.add_class(node3, "only-print");
        node3.style.position = "absolute";
        node3.style.top = 0;
        node3.style.right = 0;
        node3.style.zIndex = 99;
        node3.innerHTML = (slides.length);
        var potentialTitles = div.getElementsByTagName("div");
        for (var j = 0; j < potentialTitles.length; ++j) {
          if (w3c_slidy.has_class(potentialTitles[j], "titlebox")) {
            potentialTitles[j].appendChild(node3);
          }
        }
      }
...
//        location.href = this.href;
  ...
  check_location: function () {
    ...
    // chs: negative time is interesting, too
  ...
  // chs: en/disable the display of remaining time
  set_display_time: function (display_time) {
    this.display_time = display_time;
  },

  get_timer: function () {
    // chs: only display time if enabled
    if (!w3c_slidy.display_time) {
      return "";
    }
  ...
  previous_slide: function (incremental) {
    if (!w3c_slidy.view_all)
    {
      ...
      else if (w3c_slidy.slide_number > 0)
      {
        ...
        slide = w3c_slidy.slides[w3c_slidy.slide_number];
        var shown_nodes = w3c_slidy.set_visibility_all_incremental("visible");
  ...
  last_slide: function () {
    if (!w3c_slidy.view_all)
    {
          ...
          w3c_slidy.slide_number < w3c_slidy.slides.length - 1)
      {
         ...
         w3c_slidy.slide_number = w3c_slidy.slides.length - 1;
         slide = w3c_slidy.slides[w3c_slidy.slide_number];
         shown_nodes = w3c_slidy.set_visibility_all_incremental("visible");
         w3c_slidy.last_shown = w3c_slidy.previous_incremental_item(null);
  ...
  // chs: register the show-slide-handler, function (slide) { ... }
  set_show_slide_handler: function (handler) {
    this.show_slide_handler = handler;
  },

  // chs: register the hide-slide-handler, function (slide) { ... }
  set_hide_slide_handler: function (handler) {
    this.hide_slide_handler = handler;
  },

  // chs: register the show-node-handler, function (slide, node) { ... }
  set_show_node_handler: function (handler) {
    this.show_node_handler = handler;
  },

  // chs: register the hide-node-handler, function (slide, node) { ... }
  set_hide_node_handler: function (handler) {
    this.hide_node_handler = handler;
  },

  // chs: register the resize-font-handler, function (slide) { ... }
  set_resize_handler: function (handler) {
    this.resize_handler = handler;
  },

  show_slide: function (slide, doNotCallHandler) {
    this.sync_background(slide);
    window.scrollTo(0,0);
    this.remove_class(slide, "hidden");
    // chs: call the handler
    if (this.show_slide_handler != null && !doNotCallHandler) {
      this.show_slide_handler(slide);
    }
    // chs: vertically centering things isn't easy without absolute measures
    // (these weird -moz-boxes etc. are not intended for that purpose, I think)
    // so we do this here
    var restHeight = 0;
    var vbody = null;
    var hbody = null;
    for (var node = slide.firstChild; node; node = node.nextSibling) {
      if (w3c_slidy.has_class(node, "hcenter")) { hbody = node; }
      if (w3c_slidy.has_class(node, "vcenter")) { vbody = node; }
      else if (isFinite(node.clientHeight)) { restHeight += node.clientHeight; }
    }
    if (hbody) {
      var sw = slide.clientWidth;
      var bw = hbody.clientWidth;
      var left = (sw - bw) / 2;
      hbody.style.left = left +"px";
    }
    if (vbody) {
      var sh = slide.clientHeight;
      var bh = vbody.clientHeight;
      var th = (this.toolbar && isFinite(this.toolbar.clientHeight)) ? this.toolbar.clientHeight : 0;
      var rh = restHeight;
      var top = rh + (sh - rh - bh - th) / 2;
      vbody.style.top = top +"px";
    }
  },

  hide_slide: function (slide, doNotCallHandler) {
    this.add_class(slide, "hidden");
    // chs: call the handler
    if (this.hide_slide_handler != null && !doNotCallHandler) {
      this.hide_slide_handler(slide);
    }
  },
  ...
  resized: function () {
     ...
     if (width != w3c_slidy.last_width || height != w3c_slidy.last_height)
     {
       ...
       {
         var slide = w3c_slidy.slides[w3c_slidy.slide_number];
         w3c_slidy.hide_slide(slide, true);
         w3c_slidy.show_slide(slide, true);
       }
...
// invoked via F key
  ...
  key_down: function (event) {
    ...
    if (key == 34) // Page Down
    {
      ...
      // chs: PgDn is (allegedly) sent by presenters for next (sub-) slide:
      w3c_slidy.next_slide(true);
      return w3c_slidy.cancel(event);
    }
  ...
  reveal_next_item: function (node) {
    ...
    if (node && node.nodeType == 1) { // an element
      w3c_slidy.remove_class(node, "invisible");
      if (this.show_node_handler != null) {
        this.show_node_handler(node);
      }
    }
  ...
  hide_previous_item: function (node) {
    if (node && node.nodeType == 1)  { // an element
      w3c_slidy.add_class(node, "invisible");
      if (this.hide_node_handler != null) {
        this.hide_node_handler(node);
      }
    }
  ...
  // chs: the `stop-incremental' class can be used to force the traversal to
  // stop which is helpful when the element gets dynamically added children
  // (e.g., because it contains some animations using document.createElement())
  next_node: function (root, node) {
    if (node == null)
      return root.firstChild;

    if (node.firstChild && !w3c_slidy.has_class(node, "at-once"))
  ...
  previous_node: function (root, node) {
    if (node == null)
    {
      ...
      if (node)
      {
        while (node.lastChild && !w3c_slidy.has_class(node, "at-once"))
          node = node.lastChild;
      }
